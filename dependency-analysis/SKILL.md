...
INPUT (optional): a specific dependency or category to focus on (e.g., "react ecosystem", "dev deps only", "security").
...
HALLUCINATED / NON-EXISTENT PACKAGES (AI-specific, check FIRST):
- Verify EVERY package in the manifest actually exists on its registry, plus
  every package the AI recommends adding:
  - npm: `npm view <pkg> name version time.modified` (or `npm view <pkg>` and
    check `dist-tags`, `maintainers`, `time`)
  - Python: `pip index versions <pkg>` or `python -m pip download <pkg> --no-deps -d /tmp/probe`
  - Go: `go list -m -versions <module>`
  - Rust: `cargo search <name> --limit 3`
  - Java: check Maven Central / repo1.maven.org search API
- A package that 404s on its registry is a HALLUCINATED package: the AI
  generated a plausible name. NEVER install it, and do not "fix" the name by
  guessing — search for the intended real package and confirm intent.
- For every existing package, assess:
  | Check | Risk signal |
  |---|---|
  | Downloads | < 1000/week (or single-digit monthly) = low trust |
  | Maintainer account age | Newly registered account = high risk (squatting) |
  | Last publish | Years stale OR days fresh with no history = both suspicious |
  | Name similarity | Typosquatting candidate: differs from a popular package by one character (`cross-env` vs `crossenv`) |
  | Install scripts | `postinstall`/`preinstall` in a small or unknown package = arbitrary code on install |
...
- Install a package before verifying it exists on the registry (hallucinated names are the top AI supply-chain risk).

NEXT STEPS:
- Apply the safe updates and run the tests to verify.
- Measure the size impact of dependency changes (bundle analysis) if relevant.
- Run a broader security review beyond dependencies if needed.
- Remove dead code alongside unused dependencies if found.
...
After producing output, record execution metadata for skill improvement signals (optional).

Check if a project memory directory exists (e.g. `.hermes/` or a `skill-telemetry.md` convention in use):
