...
## Verifying Work in a Shared Workspace

When implementers share one working tree (single-branch strategy), your typecheck/test/build runs will frequently catch sibling agents **mid-edit**. Do not treat every red run as your bug, and never start "fixing" errors in files you don't own.

1. **Attribution first**: run `git status --short`. Files already modified before your session (or modified by others) that fail are not yours. Errors in YOUR owned files are the only ones you must fix.
2. **Error-set drift detection**: if a check fails, re-run it after ~15–30s. If the error set shifts (different files, different lines, different counts), another agent is actively writing — the failures are transient mid-edit states. Wait and re-run until the error set is stable or clean.
3. **Final gate is the converging run**: keep re-running until a full clean pass (e.g. `npx tsc --noEmit` → exit 0). Report the clean run as the verification evidence; note the transient noise in the report's concerns section.
4. **Respect ownership under pressure**: even when tsc is red on a contract file you're forbidden to touch, do not patch it — the brief's MUST NOT DO outranks a green build. Document the blocker; it usually resolves itself as the owning agent finishes.
5. Distinguish build artifacts from hand edits: a build step that regenerates files (e.g. Tailwind CSS, `dist/`) will dirty other paths in `git status` — that's expected, not scope creep.

See `references/shared-workspace-verification.md` for a full worked example (error sequences, commands, tooling quirks).

## Related Skills

- team-composition-patterns (wshobson/agents ecosystem) — choose the right team size and agent types before decomposing work
- team-communication-protocols (wshobson/agents ecosystem) — coordinate integration handoffs and plan approvals between implementers
