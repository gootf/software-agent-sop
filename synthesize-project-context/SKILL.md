...
> Built from the S01–S22 trial run finding F1 (2026-08-11): `project-context` provides the carrier (file format + add/cleanup/display lifecycle) but no multi-source synthesis step. This skill is that step. It complements — never replaces — `project-context` (which keeps owning the file format and lifecycle).
