...
>
> **Format philosophy**: the YAML reads like the mermaid `flowchart TB` blocks in the reconstruction docs — node blocks carry identity, a single `edges:` list carries ALL arrows (with `label:` as the arrow text), and routing semantics (branch condition / pass-fail / loop) are edge properties, not hidden fields.
